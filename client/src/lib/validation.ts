...
export const emailSchema = string({ required_error: "Email is required" }).min(1, "Email is required")
...
export const SignUpSchema = object(
    {
        name: string().min(3, "Name is required").max(255, "Name can't contain more than 255 Characters"),
        email: emailSchema,
...
export const BlogSchema = object({
    title: string().min(1, "Title is required").max(255, "The title cann't contain more than 255 Characters"),
    image: string().min(1, "Image is required"),
    shortDescription: string({ message: "Short Description is required" }).min(40, "The short description must contains 40 Characters").max(160, "The short description cann't contain more than 160 Characters"),
    content: string(),
